...
int main() {
	...
	char *message,*emessage,*dmessage;		//declare the variables that will contain characters
 
	int i,j=0,k,key,temp		// initialise a temp value and the variables used to  
	...
	for (i=0;message[i]!=NULL;i++) {		//using null rovides a pointer without having values to store
 
		//printf("%c ",message[i]);
 
		if(message[i]==' ')
 
		    emessage[j++]=message[i]; else {		//use for counter to sort array 
